...
function Dashboard() {
  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-2 md:grid-rows-3 lg:grid-cols-4
      xl:grid-cols-5 gap-5 mt-10'>
        {
          fileList?.length>0 ? fileList?.map((file,index)=>(
          <Link key={file.fileId || index} href={'/workspace/'+file.fileId}>

            <div key={index} className='flex p-5 shadow-md rounded-md
             flex-col items-center justify-center border
